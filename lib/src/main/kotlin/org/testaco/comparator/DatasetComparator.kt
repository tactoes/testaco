...
class DatasetComparator(val schema: TestacoSchema) {
  ...
  private fun compareRows(tableName: String, dbRow: Row, refRow: Row): List<CRowResult> {
    ...
    return dbColNames.minus(refColNames).map { columnName: String -> CColumnMissingResult(tableName, columnName, "Column with name ${columnName} is missing from reference data set row with id ${dbRow.pk()}")}
      .plus(refColNames.minus(dbColNames).map { columnName: String -> CColumnMissingResult(tableName, columnName, "Column with name ${columnName} is missing from database data set row with id ${dbRow.pk()}") })
